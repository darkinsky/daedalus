...
graph TD
    ...
    Main --> Config[config.rs<br/>YAML 配置加载]
    Main --> Logging[config/logging.rs<br/>结构化日志]
    ...
    ChatAgent --> Session[agent/session.rs<br/>会话管理]
...
main()
  ...
  3. AgentConfig::from_workspace() → 从 YAML 配置文件加载 + SOUL 文件
