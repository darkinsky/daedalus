...
> 最后更新：2026-04-17
> 来源：存量代码分析 + 代码审查改进 + 记忆系统重构代码审查 + 架构审查优化 + 代码质量审查优化 + **MemPalace 架构审查**
> 置信度：高
...
7. **God Object 拆分用跨文件 `impl` 扩展**：当一个 struct 承担过多职责（>500 行或 >3 个不相关职责）时，使用 Rust 的跨文件 `impl` 扩展模式拆分——struct 定义在主文件，各职责的 `impl` 块分布在独立文件中。字段使用 `pub(super)` 可见性。例如 `Palace` struct 在 `palace.rs`，KG 查询在 `knowledge_graph.rs`，图遍历在 `graph.rs`
8. **Feature-complete 但未集成的模块用 `#[allow(dead_code)]`**：当模块 100% 复刻了外部参考实现的全部功能，但部分功能尚未集成到主 pipeline 时，使用 `#[allow(dead_code)]` 注解保留完整实现，而非删除代码。注解应加在具体的类型/方法/常量上，而非模块级别
