...
> 最后更新：2026-04-15
> 来源：存量代码分析 + 代码审查改进 + 工具事件/并行化迭代 + 记忆系统重构 + Skill 功能实现 + 模块化重构 + Bash 工具 + 代码质量审查优化
...
`AgentMode` trait 通过 supertrait `AgentMetadata` 分离了核心行为和只读元数据：
...
pub trait AgentMetadata {
    ...
    fn tool_infos(&self) -> Vec<ToolInfo> { vec![] }
    ...
    fn subagent_infos(&self) -> Vec<SubagentInfo> { vec![] }
    fn subagent_count(&self) -> usize { 0 }
}

/// 核心行为
#[async_trait]
pub trait AgentMode: AgentMetadata + Send + Sync {
    async fn chat(&mut self, user_input: &str, on_tool_event: Option<&ToolEventCallback>) -> Result<ChatResponse>;
...
**设计模式**：supertrait 分离。`AgentMetadata` 聚焦只读元数据（CLI 展示），`AgentMode` 聚焦核心行为（chat、session、shutdown）。`render.rs` 只依赖 `&dyn AgentMetadata`，不需要 `&mut dyn AgentMode`。[置信度：高]
...
用户消息 → LLM 请求（带工具定义 + 历史）
  → LLM 返回 tool_calls?
    YES → 发送 RoundStart 事件（round_number 从 1 开始）
...
`main()` 通过 `bootstrap()` 和 `build_agent()` 两个辅助函数完成初始化，自身仅负责模式分发：
...
main()
  ...
  │   ├─ 初始化日志（返回 LogGuard）
  │   └─ build_agent()
  │       ├─ 连接 MCP 服务器（如非 bare 模式）
  │       ├─ 创建 LLM Provider
  │       ├─ 创建 ChatAgent
  │       ├─ 加载 Skills + Subagents
  │       └─ 应用工具过滤 + max-turns
  └─ 模式分发
      ├─ --print → cli::run_print() → agent.shutdown()
      └─ 默认 → cli::run_interactive()
```

**设计动机**：将 `main()` 的 CC 从 ~8 降到 ~3，每个函数职责单一。`LogGuard` 作为 `bootstrap()` 的返回值传出，确保在 `main()` 作用域内持有直到进程退出。
