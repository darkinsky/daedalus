...
|------|:-----------:|:------------:|:----------------:|:------------:|
| microCompact (tool result 截断) | ✅ | ✅ (类似) | ✅ | ⚠️ 部分（仅 tool_loop 内） |
| AI 摘要压缩 | ✅ | ✅ | ✅ | ✅ |
| 增量压缩 (boundary) | ✅ | ❌ | ❌ | ❌ |
| 保留段语义标记 | ✅ | ⚠️ | ❌ | ⚠️ (仅最近 N 条) |
| Partial compact | ✅ | ❌ | ❌ | ❌ |
...
| microCompact | 🔴 P0 | 🔥🔥🔥🔥🔥 | 低 |
| 熔断机制 | 🟠 P1 | 🔥🔥 | 极低 |
| CJK-aware token 估算 | 🟠 P1 | 🔥🔥🔥 | 低 |

### Phase 2: 架构优化（3-5 天）

| 任务 | 优先级 | 预期收益 | 实现难度 |
|------|:------:|:--------:|:--------:|
| compactBoundary 增量压缩 | 🟡 P2 | 🔥🔥🔥 | 中 |
| 多级阈值 | 🟡 P2 | 🔥🔥 | 低 |
...
| 任务 | 优先级 | 预期收益 | 实现难度 |
|------|:------:|:--------:|:--------:|
| preservedSegment 语义标记 | 🟢 P3 | 🔥🔥 | 高 |
| Partial compact | 🟢 P3 | 🔥 | 中 |
...
|------|-------------|----------|
| **压缩哲学** | "能不调 LLM 就不调 LLM" — microCompact 用字符串替换解决大部分问题 | "统一用 LLM 做摘要" — 跳过了最轻量的层 |
| **阈值策略** | 激进（93% 才触发），留更多空间给对话 | 保守（80% 就触发），更早压缩 |
| **增量性** | 支持增量压缩（boundary），成本递减 | 全量压缩，成本恒定 |
| **容错性** | 熔断机制，3 次失败停止 | 无熔断，每轮重试 |
