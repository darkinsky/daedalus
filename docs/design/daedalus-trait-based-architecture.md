...
> 最后更新：2026-04-17
> 来源：存量代码分析 + 代码审查改进 + 工具事件/并行化迭代 + 记忆系统重构 + Skill 功能实现 + Workspace 系统实现 + 架构审查优化 + **六策略互斥记忆架构** + **MemPalace Memory**
> 置信度：高
...
## MemPalace Memory 设计决策

### God Object 拆分模式

**背景**：架构审查发现 `palace.rs` 是 1416 行的 God Object，承担 7 个不同职责（数据模型、KG 查询、图遍历、日记、身份、种子导入、空间操作）。

**决策**：将 `palace.rs` 拆分为 6 个文件，使用 Rust 的 `impl` 跨文件扩展模式——`Palace` struct 定义在 `palace.rs`，各职责的 `impl Palace` 块分布在 `knowledge_graph.rs`、`graph.rs`、`diary.rs`、`identity.rs` 中。

**权衡**：
- ✅ 每个文件职责单一，可独立理解和测试
- ✅ 利用 Rust 的 `impl` 跨文件扩展，无需引入 trait 或委托模式
- ✅ `palace.rs` 从 1416 行降到 ~990 行
- ⚠️ 需要 `pub(super)` 可见性让同模块内的其他文件访问 Palace 字段

### Palace 字段封装（pub → pub(super)）

**背景**：`Palace` 的所有字段（wings, tunnels, entities, triples, diary, identity）都是 `pub`，外部代码可直接修改内部状态，绕过业务规则（如三元组去重检查）。

**决策**：将所有字段改为 `pub(super)`，通过方法暴露操作接口。同时为 `MemPalaceStore` 的 `palace`、`drawers`、`closets` 字段也改为 `pub(super)`，并新增 `uncloseted_drawers()` 等封装方法。

**权衡**：
- ✅ 外部代码无法绕过去重检查直接 push 到 triples
- ✅ Store 的内部数据结构不再暴露给 Memory 层
- ⚠️ 同模块内的文件仍可直接访问（`pub(super)`），但这是合理的——它们是同一个模块的内部实现

### reflect_on_turn 编排重构

**背景**：`reflect_on_turn` 是一个 ~100 行的 async 方法，包含 9 个步骤，每个步骤都有错误处理分支，圈复杂度过高。

**决策**：将每个步骤提取为独立的私有方法（`normalize_and_dedup`、`classify_turn`、`store_classified_turn`、`store_in_chroma`、`maybe_generate_closet`、`pre_retrieve_context`），`reflect_on_turn` 只做编排。

**权衡**：
- ✅ 每个步骤可独立理解和测试
- ✅ `reflect_on_turn` 主体缩短为 ~30 行，流程一目了然
- ✅ 错误处理逻辑内聚在各步骤方法中

### BFS 队列 VecDeque 优化

**背景**：`traverse()` 使用 `Vec` 模拟 BFS 队列，`frontier.remove(0)` 是 O(n) 操作。

**决策**：改用 `VecDeque`，`pop_front()` 是 O(1)。

### 共享停用词表提取

**背景**：`entity_detector.rs` 和 `dialect.rs` 各自定义了 ~90-100 个停用词，高度重叠但不完全相同。

**决策**：提取到 `stopwords.rs` 作为共享的 `Lazy<HashSet<&str>>`，两个模块都引用同一份停用词表。

**权衡**：
- ✅ 消除了维护两份几乎相同停用词表的负担
- ✅ 修改停用词只需改一处

### format_context_sections 提取

**背景**：`retrieve_context()` 方法融合 6 种检索信号后，还包含 ~30 行的格式化逻辑（将各信号结果拼接为 Markdown），导致方法过长且职责不单一。

**决策**：提取 `format_context_sections()` 纯函数，接收各信号结果，返回格式化的 Markdown 字符串。`retrieve_context()` 只负责信号收集和编排。

**权衡**：
- ✅ `retrieve_context()` 职责更单一（信号收集 vs 格式化分离）
- ✅ `format_context_sections()` 是纯函数，易于测试

### 死代码清理策略

**背景**：MemPalace 100% 复刻了 Python 原版的全部功能，但部分功能（如 AAAK 方言压缩、实体检测、KG 查询 API）尚未集成到主 pipeline 中，产生大量 dead_code warnings。

**决策**：为所有属于公共 API 但尚未集成的类型/方法/常量添加 `#[allow(dead_code)]` 注解，而非删除代码。这些是 feature-complete 的实现，等待未来集成。

**权衡**：
- ✅ 保留了完整的功能实现，随时可集成
- ✅ 消除了编译警告噪音
- ⚠️ 增加了少量维护负担（需要在集成时移除 `#[allow(dead_code)]`）

## 对标 Claude Code 工具集设计决策

### grep_search 内置 rg 二进制决策

**背景**：对标 Claude Code 的 `GrepSearch` 工具，需要为 Daedalus 添加文件内容搜索能力。经历了三次方案迭代：
...
**决策**：选择方案 C。将 ripgrep 14.1.1 的静态链接二进制（x86_64-linux-musl，6.3MB）放在项目 `bin/` 目录下，`grep_search` 工具通过 `tokio::process::Command` 调用。二进制路径通过 `once_cell::Lazy` 缓存，只解析一次。

**rg 二进制发现优先级**：
1. `<exe_dir>/bin/rg` 及祖先目录（循环遍历最多 3 层，适配 `cargo run` 的 `target/debug/` 布局）
2. `<cwd>/bin/rg`（开发时从项目根目录运行）
3. 系统 PATH 上的 `rg`（兜底）

**权衡**：
- ✅ 零 Rust crate 依赖，不增加编译时间
- ✅ 性能等同原生 rg（相同的二进制）
- ✅ 自动尊重 `.gitignore`、跳过隐藏目录和二进制文件
- ✅ 30 秒超时防止搜索挂起
- ⚠️ 需要在 CI/CD 或首次部署时下载 rg 二进制
- ⚠️ `bin/` 在 `.gitignore` 中，二进制不入库

### edit_file 精确编辑工具决策

**背景**：对标 Claude Code 的 `Edit` 工具。原有的 `write_file` 只支持全量覆写，无法进行精确的 search-and-replace 编辑，LLM 需要重写整个文件才能修改一行。

**决策**：新增 `edit_file` 工具，实现精确的 search-and-replace 编辑，具有安全特性：
- 精确匹配（`old_string` 必须完全匹配文件中的内容）
- 消歧（如果 `old_string` 匹配多处，返回错误要求提供更多上下文）
- 防误操作（`old_string` 和 `new_string` 不能相同）

**权衡**：
- ✅ LLM 只需指定要修改的片段，不需要重写整个文件
- ✅ 安全特性防止误操作
- ✅ 自动创建父目录

### multi_edit 批量编辑工具决策

**背景**：对标 Claude Code 的 `MultiEdit` 工具。单文件多处编辑时，多次调用 `edit_file` 效率低且中间状态不一致。

**决策**：新增 `multi_edit` 工具，支持单文件多处编辑的原子操作：
- 编辑按顺序执行
- 任一编辑失败时自动回滚到原始内容
- 支持 `replace_all` 选项
...
### ToolEvent 从 agent 迁移到 tools 模块决策

**背景**：架构审查发现 `subagent/tool.rs` 和 `subagent/runner.rs` 依赖 `crate::agent::{ToolEvent, ToolEventCallback}`，形成 `subagent` → `agent` 的反向依赖。在分层架构中，同层模块不应互相依赖。

**决策**：将 `ToolEvent` 枚举和 `ToolEventCallback` 类型别名的 canonical 定义从 `agent/mod.rs` 迁移到 `tools/mod.rs`（共享基础设施层）。`agent/mod.rs` 通过 `pub use crate::tools::{ToolEvent, ToolEventCallback}` re-export 保持向后兼容。

**权衡**：
- ✅ `subagent/` 不再依赖 `agent/`，依赖图变为严格单向
- ✅ 所有现有的 `use crate::agent::ToolEvent` 路径仍然有效（通过 re-export）
- ✅ 语义上 ToolEvent 属于工具执行层，放在 `tools` 更合理

### create_memory_factory 提取到 memory 模块决策

**背景**：架构审查发现 `ChatAgent` 承担了过多职责（722 行），其中 `create_memory_factory()` 方法包含 6 路 match 分支（每种记忆策略一个），是 ChatAgent 中圈复杂度最高的方法。

**决策**：将 `create_memory_factory()` 和辅助函数 `sliding_window_factory()` 从 `ChatAgent` 提取到 `memory/mod.rs` 作为独立的 `pub fn create_memory_factory()` 函数。同时将 MemPalace 的 `panic!` 替换为优雅降级（fallback to sliding_window）。

**权衡**：
- ✅ `ChatAgent` 从 722 行降到 620 行（-14%）
- ✅ 记忆策略工厂选择逻辑归属到 `memory` 模块，符合单一职责原则
- ✅ `ChatAgent` 不再需要导入 6 个具体 Factory 类型
- ✅ MemPalace 配置错误不再导致 panic，统一为 fallback 模式

### format_tool_event_lines 提取消除重复渲染逻辑

**背景**：代码设计审查发现 `cli/render.rs` 的 `tool_event()` 和 `cli/print_runner.rs` 的 `build_text_stderr_callback()` 对 `ToolEvent` 的 6 个变体做了几乎相同的 match 渲染，唯一区别是输出目标（stdout vs stderr），约 180 行重复代码。

**决策**：提取 `format_tool_event_lines(event) -> Vec<String>` 纯函数，返回预格式化的行列表。`tool_event()` 输出到 stdout，`build_text_stderr_callback()` 输出到 stderr。

**权衡**：
- ✅ 消除了 ~180 行重复代码
- ✅ `print_runner.rs` 从 343 行降到 258 行（-25%）
- ✅ 纯函数设计，消费者自行决定输出目标

### Memory 子模块 re-export 清理决策

**背景**：代码设计审查发现 4 个 memory 子模块的 `mod.rs` 中有 15 个 `pub use` re-export 但从未被外部使用，产生编译 warnings。

**决策**：
- 只保留被外部使用的 Factory 类型的 `pub use` re-export
- 仅测试使用的类型改为 `#[cfg(test)] pub(crate) use`（如 `SlidingWindowConfig`、`HistoryEntry` 等）
- 其他内部类型移除 re-export

**权衡**：
- ✅ 编译 warnings 从 18 降到 0
- ✅ 模块的公共 API 表面积更小，封装性更好
- ✅ `#[cfg(test)]` 条件编译确保测试仍可访问内部类型
...
| 日期 | 变更 | 来源 |
|------|------|------|
| 2026-04-17 | 新增对标 Claude Code 工具集设计决策（grep_search 内置 rg 二进制、edit_file 精确编辑、multi_edit 批量编辑、ToolEvent 迁移到 tools 模块、create_memory_factory 提取到 memory 模块、format_tool_event_lines 提取、Memory 子模块 re-export 清理） | 对标 Claude Code 工具实现 + 架构审查优化 + 代码设计审查优化 |
| 2026-04-17 | 新增 MemPalace Memory 设计决策（God Object 拆分、Palace 字段封装、reflect_on_turn 编排重构、BFS VecDeque 优化、共享停用词表、format_context_sections 提取、死代码清理策略） | MemPalace Memory 实现 + 架构审查优化 |
