...
## SubagentToolBase 共享状态提取决策

**背景**：`SubagentTool` 和 `TeamTool` 持有相同的三个 `Arc` 字段（registry、runner、shared_callback），存在结构性重复。

**决策**：提取 `SubagentToolBase` 共享结构体，封装三个 `Arc` 字段和 `read_callback()`、`emit_start()`、`emit_complete()` 方法。两个 Tool 通过 `base` 字段引用。

**权衡**：
- ✅ 消除了字段重复和事件发射逻辑重复
- ✅ 工厂函数 `build_subagent_tool()` / `build_team_tool()` 从 registry 移到 tool.rs，消除了双向依赖
- ⚠️ 引入了一层间接（`self.base.registry`），但可读性更好

## SharedEventCallback 运行时回调注入决策

**背景**：`BuiltinTool::execute()` 的签名是固定的（不接受额外参数），但 subagent 执行时需要将 ToolEvent 回调传递给 runner。回调在 REPL 中每次 chat 调用时才创建（绑定到 spinner）。

**决策**：使用 `Arc<RwLock<Option<ToolEventCallback>>>` 作为共享回调容器。REPL 在每次 chat 调用前通过 `set_subagent_event_callback(Some(cb))` 设置，调用后通过 `set_subagent_event_callback(None)` 清除。`SubagentTool` 在执行时通过 `read_callback()` 读取。

**权衡**：
- ✅ 不需要修改 `BuiltinTool` trait 签名
- ✅ 回调可在运行时动态注入/清除
- ⚠️ 引入了 `RwLock` 开销，但对于工具调用这种 IO 密集操作可忽略
